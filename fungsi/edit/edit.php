...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['suplayer'])) {
        $id_suplayer = htmlentities($_POST['id_suplayer']);
        $nama_suplayer = htmlentities($_POST['nama_suplayer']);
        $alamat_suplayer = htmlentities($_POST['alamat_suplayer']);
        $tlp = htmlentities($_POST['tlp']);
        $data = array($nama_suplayer, $alamat_suplayer, $tlp, $id_suplayer);
        $sql = 'UPDATE suplayer SET nama_suplayer=?, alamat_suplayer=?, tlp=? WHERE id_suplayer=?';
        $row = $config->prepare($sql);
        $row->execute($data);
        echo '<script>window.location="../../index.php?page=suplayer&success-edit=edit-data"</script>';
    }
    ...
    if (!empty($_GET['login'])) {
        ...
        $row = $config->prepare($sql);
        $row->execute($data);
        echo '<script>window.location="../../index.php?page=login&uid=' . $id . '&success-edit=edit-data"</script>';
    }
    ...
    if (!empty($_GET['barang'])) {
        ...
        $data[] = $tgl;
        $data[] = $id;
        $sql = 'UPDATE barang SET id_kategori=?,id_suplayer=?, nama_barang=?, merk=?,
